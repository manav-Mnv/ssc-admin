...
function renderModalData() {
  ...
  keys.forEach((key) => {
    // Skip internal IDs if needed
    if (key === "id") return;
    
    // Tab categorization check
    if (currentTab === "personal" && !CATEGORIES.personal.includes(key)) return;
    if (currentTab === "swift" && !CATEGORIES.swift.includes(key)) return;
    ...
    // Check if label should take full grid width (long comments/text)
